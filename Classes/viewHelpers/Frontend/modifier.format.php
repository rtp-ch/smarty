<?php
    ...
    {
        // Gets the typoscript configuration for parseFunc, default is "lib.parseFunc_RTE"
        $parseFuncSettings = $setup ? '<' . $setup : $GLOBALS['TSFE']->tmpl->setup['lib.']['parseFunc_RTE.'];

        // Process the content with the defined parseFunc configuration
        if ($setup) {
            $formatgedContent = $GLOBALS['TSFE']->cObj->parseFunc($content, '', $parseFuncSettings);
