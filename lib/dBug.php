...
/**
 * Modified version of dBug (http://dbug.ospinto.com) for Smarty debug console:
 * - Added magic function __toString() to make compatible with PHP Version > 5.2
 * - Output is appended to $this->prettyDump instead of echoed
 * - Backtrace greps for smarty_modifier_debug_var instead of dBug
 *
...
class dBug {
	...
	function getVariableName() {
		...
		for($i=count($arrBacktrace)-1; $i>=0; $i--) {
			...
			if(array_key_exists("function", $arrCurrent) &&
				(in_array($arrCurrent["function"], $arrInclude) || (0 != strcasecmp($arrCurrent["function"], "smarty_modifier_debug_var"))))
				continue;
			$arrFile = $arrCurrent;
			break;
		}
		...
		if(isset($arrFile)) {
			...
			//find call to smarty_modifier_debug_var
			preg_match('/smarty_modifier_debug_var\s*\(([^)]+)\)/i', $code, $arrMatches);
			$args = explode(',',$arrMatches[1]);
			$varName = ($args[3])?$args[3]:$args[0];
	...
	function makeTableHeader($type,$header,$colspan=2) {
		...
		$this->prettyDump .= "<table cellspacing=2 cellpadding=3 class=\"dBug_".$type."\">
				<tr>
	...
	function checkType($var) {
		switch(gettype($var)) {
			...
			default:
				...
				$this->prettyDump .= "<table cellspacing=0><tr>\n<td>".$var."</td>\n</tr>\n</table>\n";
				break;
		}
	}
		...
		if(is_array($var)) {
			foreach($var as $key=>$value) {
				...
				if(in_array(gettype($value),$this->arrType))
					...
					$this->prettyDump .= $value;
				}
				$this->prettyDump .= $this->closeTDRow();
			}
		}
		else $this->prettyDump .= "<tr><td>".$this->error("array").$this->closeTDRow();
		array_pop($this->arrHistory);
		$this->prettyDump .= "</table>";
	}
	...
	function varIsObject($var) {
		...
		if(is_object($var)) {
			...
			foreach($arrObjVars as $key=>$value) {
				...
				if(in_array(gettype($value),$this->arrType))
					$this->checkType($value);
				else $this->prettyDump .= $value;
				$this->prettyDump .= $this->closeTDRow();
			}
			$arrObjMethods=get_class_methods(get_class($var));
			foreach($arrObjMethods as $key=>$value) {
				$this->makeTDHeader("object",$value);
				$this->prettyDump .= "[function]".$this->closeTDRow();
			}
		}
		else $this->prettyDump .= "<tr><td>".$this->error("object").$this->closeTDRow();
		array_pop($this->arrHistory);
		$this->prettyDump .= "</table>";
	}
	...
	function varIsResource($var) {
		...
		switch(get_resource_type($var)) {
			...
			case "xml":
				...
				break;
			default:
				$this->prettyDump .= get_resource_type($var).$this->closeTDRow();
				break;
		}
		$this->prettyDump .= $this->closeTDRow()."</table>\n";
	}
	...
	function varIsDBResource($var,$db="mysql") {
		...
		$this->prettyDump .= "<tr><td class=\"dBug_resourceKey\">&nbsp;</td>";
		for($i=0;$i<$numfields;$i++) {
			...
			$this->prettyDump .= "<td class=\"dBug_resourceKey\" title=\"".$field_header."\">".$field_name."</td>";
		}
		$this->prettyDump .= "</tr>";
		for($i=0;$i<$numrows;$i++) {
			$row=call_user_func($db."_fetch_array",$var,constant(strtoupper($db)."_ASSOC"));
			$this->prettyDump .= "<tr>\n";
			$this->prettyDump .= "<td class=\"dBug_resourceKey\">".($i+1)."</td>";
			for($k=0;$k<$numfields;$k++) {
				$tempField=$field[$k]->name;
				$fieldrow=$row[($field[$k]->name)];
				$fieldrow=($fieldrow=="") ? "[empty string]" : $fieldrow;
				$this->prettyDump .= "<td>".$fieldrow."</td>\n";
			}
			$this->prettyDump .= "</tr>\n";
		}
		$this->prettyDump .= "</table>";
	...
	function varIsGDResource($var) {
		...
		$this->prettyDump .= imagesx($var).$this->closeTDRow();
		$this->makeTDHeader("resource","Height");
		$this->prettyDump .= imagesy($var).$this->closeTDRow();
		$this->makeTDHeader("resource","Colors");
		$this->prettyDump .= imagecolorstotal($var).$this->closeTDRow();
		$this->prettyDump .= "</table>";
	}
	...
	function varIsXmlResource($var) {
		...
		//if xml is not a file, attempt to read it as a string
			...
			if(!is_string($var)) {
				$this->prettyDump .= $this->error("xml").$this->closeTDRow()."</table>\n";
				return;
			}
			$data=$var;
	...
	function xmlStartElement($parser,$name,$attribs) {
		...
		$this->xmlSData[$this->xmlCount].='$this->prettyDump .= "<strong>'.$this->xmlName[$this->xmlCount].'</strong>".$this->closeTDRow();';
		...
		if(count($attribs)>0)
			...
			$this->xmlSData[$this->xmlCount].='$this->prettyDump .= "&nbsp;";';
		$this->xmlSData[$this->xmlCount].='$this->prettyDump .= $this->closeTDRow();';
		$this->xmlCount++;
	}
	...
	function xmlEndElement($parser,$name) {
		for($i=0;$i<$this->xmlCount;$i++) {
			...
			$this->prettyDump .= (!empty($this->xmlCData[$i])) ? $this->xmlCData[$i] : "&nbsp;";
			$this->prettyDump .= $this->closeTDRow();
			$this->makeTDHeader("xml","xmlComment");
			$this->prettyDump .= (!empty($this->xmlDData[$i])) ? $this->xmlDData[$i] : "&nbsp;";
			$this->prettyDump .= $this->closeTDRow();
